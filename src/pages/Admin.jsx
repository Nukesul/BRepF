// ... (остальной код остается без изменений до getBasePrice)

// Получение минимальной и максимальной базовой цены
const getPriceRange = (product) => {
  ...
  console.log(`Price Range for ${product.name}: ${minDiscountedPrice} - ${maxDiscountedPrice}`);
  return { min: minDiscountedPrice, max: maxDiscountedPrice };
};

// ... (остальной код до return остается без изменений)
                    ...
                    <div key={category.id} ref={(el) => (categoriesRef.current[category.id] = el)}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {categoryProducts.map((product) => {
                          const { min, max } = getPriceRange(product); // Получаем диапазон цен
                          return (
                            <div
                              key={product.id}
                              ...
                              <div className="p-4">
                                ...
                                <div className="mt-3 flex items-center justify-between">
                                  <p className="text-orange-500 font-bold text-lg">
                                    {min === max
                                      ? `${formatPrice(min)} Сом` // Если одна цена
                                      : `от ${formatPrice(min)} до ${formatPrice(max)} Сом` // Диапазон цен
                                    }
                                    {discounts.some((d) => d.product_id === product.id) && (
